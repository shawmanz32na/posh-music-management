 ...
 #> 
    ...
    Process {
        ...
        ForEach ($File in $Files) {
            ...
            $AlbumArtist = $FileMetadata.'Album artist'.Split([IO.Path]::GetInvalidPathChars()) | ForEach-Object { $_.Trim() } | Join-String -Separator ' _ '
            $Album = $FileMetadata.Album.Split([IO.Path]::GetInvalidPathChars()) | ForEach-Object { $_.Trim() } | Join-String -Separator ' _ '

            # Create the directory path
            If (-Not (Test-Path ($Destination.FullName + $AlbumArtist))) {
                If (-Not (Test-Path ($Destination.FullName + $AlbumArtist + "\" + $Album))) {
                    New-Item -Type Directory -Path ($Destination.FullName + $AlbumArtist + "\" + $Album)
                }
            }
            Else {
                Write-Output ($Destination.FullName + $AlbumArtist + "\" + $Album + " already exists")
            }

            # Move the file
            Move-Item -Path $File -Destination ($Destination.FullName + $AlbumArtist + "\" + $Album)
        }
    }
}

Export-ModuleMember -Function Move-Music